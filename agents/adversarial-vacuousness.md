...
- `stack`: `rust` | `csharp`.
- `mode`: `lock` | `pre_impl` | `post_green`. In `pre_impl` there is no implementation yet — tests will be run with stubs and should fail; your vacuousness check still applies (a test that passes against a stub is by definition vacuous, but you check the assertion shape regardless).
