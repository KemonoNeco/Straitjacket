---
name: fuzz-runner
description: Executes one fuzz harness for an assigned time budget, captures crash artifacts, and returns crash entries with traces. Mechanical role. Internal to the straitjacket plugin — invoked during the plugin's fuzz pipeline runner dispatch (parallel team capped at 2).
