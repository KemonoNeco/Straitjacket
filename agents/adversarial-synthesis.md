...
- `stack`: `rust` | `csharp`.
- `mode`: `regression-tests-phase-4a` | `tdd-phase-4` | `tdd-phase-6`. In `tdd-phase-4` there is no implementation yet → do NOT propose mutation runners (nothing to mutate). In other modes, propose them.
...
- **Skipping the mutation task list when applicable.** If `mode` is `regression-tests-phase-4a` or `tdd-phase-6` and a mutation tool is available, you must propose tasks. Skipping is a downstream failure.
