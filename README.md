...
A Claude Code multi-agent test-engineering plugin — *it does sanity tests*. Built as **composable chunks** (specialist agents + workflow stages + a Rust CLI) that **thin skills** compose. The flagship **`tdd`** skill drives new features test-first (spec → red → adversarial-on-red → green → mutation) under a savepoint discipline; a companion **`report-bug`** skill captures bugs found along the way into a tracked ledger (and optional GitHub/Jira tickets) that a later run can lift into tests — and, in its **publish mode**, bulk-publishes those findings to a team tracker with consistent formatting (a GitHub engineering-bug template and a Jira triage/decision template) and parent/child grouping for recurring defect classes. It hardens tests against four failure modes — **happy-path bias**, **vacuous assertions**, **test-mutation cheats**, and **test-contract misalignment** — using parallel specialist subagents, mutation testing, and (optionally) fuzzing, run as dynamic-Workflow stages when the `Workflow` tool is available (else direct `Agent` dispatch).
...
| `/straitjacket:report-bug` | **Capture mode:** capture a found bug to a tracked local ledger (`.straitjacket/bugs.json`), then optionally mirror it to a GitHub issue and/or Jira ticket. Local-first, opt-in remotes — file a bug *without derailing* the work in progress, and feed it back as test context later. **Publish mode:** bulk-publish many ledger/inline findings to a tracker with consistent formatting (GitHub engineering-bug template / Jira triage-decision template), parent/child grouping for recurring defect classes, and a plan-mode confirm before bulk-create. |
...
| `unit-test-author` | opus | Parallel team, unit-level test code |
...
| `audit-<lens>` (×7) | opus / sonnet | Isolated source-audit finders, one per lens. **opus**: latent-bug, security, concurrency, error-handling. **sonnet**: dead-code, doc-drift, performance |
...
The seven `audit-<lens>` finders run at `high` effort with `Read, Grep, Glob` only — the no-`Bash` restriction is their load-bearing diff/exec-isolation guarantee. Four critical/safety lenses (latent-bug, security, concurrency, error-handling) run `opus`; the three cosmetic lenses (dead-code, doc-drift, performance) run `sonnet` to save cost. See [docs/STAGES.md#specialist-agent-roster](docs/STAGES.md#specialist-agent-roster) for the full tool inventory and concurrency limits.
