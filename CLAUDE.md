...
> Why both surfaces still work unchanged: hooks call `${CLAUDE_PLUGIN_ROOT}/bin/straightjacket` (extensionless). It resolves to the **sh launcher** on Unix *and under git-bash / MSYS / Cygwin on Windows* — MSYS does **not** apply PATHEXT, so the sh launcher itself must (and now does) handle `MINGW*`/`MSYS*`/`CYGWIN*` `uname` output and dispatch the windows-msvc `.exe`. Under cmd.exe/PowerShell, PATHEXT resolves the `.cmd` launcher instead. Skills call bare `straightjacket` on PATH (same resolution). Neither references a triple. (Earlier this doc claimed only the `.cmd` handled Windows — that was wrong for the git-bash path and silently no-op'd the hooks there.)
