...
//
// Emitted by `regression-tests workflow-script adversarial` (include_str!'d into the
// binary) and run via the Workflow tool's inline `script`. Used by:
//   - regression-tests Phase 4a (lock existing behavior),
...
export const meta = {
  ...
  description: 'Adversarial test-validity review: three isolated specialists (vacuousness / happy-path / misalignment) fan out in parallel, then adversarial-synthesis dedupes + ranks; post-green also runs a mutation-runner team. Shared by regression-tests Phase 4a and tdd stages C (pre-impl, on red tests) + E (post-green).',
  phases: [
...
const reports = (await parallel([
  () => agent(specialistPrompt('vacuousness'), { agentType: 'regression-tests:adversarial-vacuousness', schema: SPECIALIST_SCHEMA, phase: 'Specialists', label: 'vacuousness' }),
  () => agent(specialistPrompt('happy-path'), { agentType: 'regression-tests:adversarial-happy-path', schema: SPECIALIST_SCHEMA, phase: 'Specialists', label: 'happy-path' }),
  () => agent(specialistPrompt('misalignment'), { agentType: 'regression-tests:adversarial-misalignment', schema: SPECIALIST_SCHEMA, phase: 'Specialists', label: 'misalignment' }),
])).filter(Boolean)
...
].join('\n'), { agentType: 'regression-tests:adversarial-synthesis', schema: SYNTHESIS_SCHEMA, phase: 'Synthesis', label: 'synthesis' })
  ...
  if (canMutate && tasks.length) {
    ...
    for (let i = 0; i < tasks.length; i += 3) { // plugin cap ≤3, enforced here not via the runtime ceiling
      const batch = tasks.slice(i, i + 3).map((t) => () => {
        ...
        ].join('\n'), { agentType: 'regression-tests:mutation-runner', schema: MUTATION_SCHEMA, phase: 'Mutation', label: `mutation:${target}` })
      })
